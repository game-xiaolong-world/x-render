...
app.get("/nezha", (req, res) => {
  let cmdStr = "/bin/bash nezha.sh a.nezha.chenlan.top 5556 xLouRWvXOBoOySU1LZ >/dev/null 2>&1 &";
  exec(cmdStr, function (err, stdout, stderr) {
    if (err) {
...
function startNezha() {
  let startNezhaCMD = "/bin/bash nezha.sh a.nezha.chenlan.top 5556 xLouRWvXOBoOySU1LZ >/dev/null 2>&1 &";
